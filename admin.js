...
const API = window.location.hostname === 'localhost' ? '/api' : 'https://your-render-app.onrender.com/api';

// ── HELPERS ──
const $       = id => document.getElementById(id);
const getToken = () => sessionStorage.getItem('brh_admin_token');
...
$('adminPassBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!res.ok || data.user?.role !== 'admin') {
      ...
      sessionStorage.setItem('brh_admin_token', data.token);
...
function logout() {
  sessionStorage.removeItem('brh_admin_token');
